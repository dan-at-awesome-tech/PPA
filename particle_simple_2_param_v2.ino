...
//Switch operation:
//once triggered, the next interruption of the prior IRPS starts to move the servo
//as the ball passes through it is unlikely to be affected by the servo just strarting to move
//hence on the following revolution, the servo should be fully in place for the switch to operate

//Turn switch function on or off
const bool switch_status=true;

//switch trigger starts as false.  Making true triggers servo movement
bool switch_trigger=false;

//IRPS immediately before Switch
int irpsb4switch=2;
...
IRPS(1,0,true, interbeam_dist_mm,beam2em_dist_mm,0.78,false), 
IRPS(2,2,true, interbeam_dist_mm,beam2em_dist_mm,0.78,false), 
IRPS(3,1,true, interbeam_dist_mm,beam2em_dist_mm,0.78,false) 
};
//***************************************************************************************
...
//Green button toggles to trigger Ejection
if (switch_status && (temp_controller_loop.button_status==b)){
  switch_trigger=true;
}
...
//trigger switch servo
if (switch_status && switch_trigger){
...
void display_main(){
  ...
  tft.setTextSize(1);
  tft.println("~ PPA! ~ particle_simple_2_param_v2");
  tft.println("");
  tft.print("  Speed");
  tft.setTextSize(2);
}
